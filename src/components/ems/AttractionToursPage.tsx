    ...
    <div className="space-y-4">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="text-text-muted text-xs border-b border-border bg-surface">
              <th className="text-left py-2.5 px-3">Attraction Name</th><th className="text-left py-2.5 px-3">Genre(s)</th><th className="text-left py-2.5 px-3">Market Tier</th><th className="text-left py-2.5 px-3">Agency</th><th className="text-left py-2.5 px-3">Active Tours</th><th className="text-left py-2.5 px-3">IAE Status</th><th />
            </tr></thead>
            <tbody>
      ...
      {pageTab === 'Tours' && (
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="text-text-muted text-xs border-b border-border bg-surface">
              <th className="text-left py-2.5 px-3">Tour Name</th><th className="text-left py-2.5 px-3">Attraction</th><th className="text-left py-2.5 px-3">Status</th><th className="text-left py-2.5 px-3">Routing Period</th><th className="text-left py-2.5 px-3">Deal Type</th><th className="text-left py-2.5 px-3">Territory DMAs</th><th />
            </tr></thead>
            <tbody>
              {filteredTours.map(t => {
      ...
      {tour && (
        <Drawer onClose={() => setSelectedTour(null)} width={640}>
          ...
          <div className="p-4 text-sm">
            {tourDrawerTab === 'Details' && <div className="space-y-2"><div><span className="text-text-muted text-xs">Deal Type: </span><span className="text-text-primary">{tour.dealType}</span></div><div><span className="text-text-muted text-xs">Routing: </span><span className="text-text-primary">{formatDate(tour.startDate)} – {formatDate(tour.endDate)}</span></div></div>}
...
function TourForm({ onSave, onCancel, initial, attractions, dmas }: { onSave: (t: Tour) => void; onCancel: () => void; initial?: Tour; attractions: Attraction[]; dmas: { id: string; name: string }[] }) {
  const [name, setName] = useState(initial?.name || '');
